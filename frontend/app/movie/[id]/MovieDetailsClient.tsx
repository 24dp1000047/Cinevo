...
export default function MovieDetailsClient({ id }: { id?: string }) {
  ...
  if (!movie) {
    return (
      <div className="pt-32 text-center text-zinc-400">
        ...
        <p className="mb-6">The film you requested could not be located.</p>
  ...
  const isBookmarked = watchlist.some((w) => w.tmdbId === movie.id && w.mediaType === 'movie');
          ...
          <div className="flex-1 text-left">
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm mb-3">
              ...
              <span className="flex items-center gap-1.5 text-zinc-300 font-medium">
                ...
                {formatMinutes(movie.runtime) || '120m'}
              </span>
          ...
          <div className="mt-16 pt-8 border-t border-white/10">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
              {similar.slice(0, 5).map((sim) => (
                <MovieCard key={sim.id} item={sim} mediaType="movie" />
              ))}
            </div>
          </div>
        )}
